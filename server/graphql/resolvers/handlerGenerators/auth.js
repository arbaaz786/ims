...
export async function getBuyer(args) {
  const { _id } = args;
  return await Buyer.findById(_id);
}
...
export async function createBuyer(args) {
  ...
  return await Buyer.create(input);
}

export async function updateBuyer(args) {
  const { _id, input } = args;
  return await Buyer.findOneAndUpdate({ _id }, input, { new: true });
}
export async function deleteBuyer(args) {
  const { _id } = args;
  return await Buyer.findOneAndRemove({ _id });
}

export async function customer(args) {
